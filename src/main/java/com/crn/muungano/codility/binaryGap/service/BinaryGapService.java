...
public class BinaryGapService {
    /*
    Title:- Find longest sequence of zeros in binary representation of an integer.
    ...
    public BinaryGapResponseDTO findBinaryGap(int num){
        // Initiate Variables
        int initialCount = 0;
        int greaterCount = 0;
        String binaryString = intToBinary(num);
        BinaryGapResponseDTO response = new BinaryGapResponseDTO();
        ...
        if (binaryString.length() <=2){
            response.setGap(0);
            System.out.println("Number is less than 2 --> "+ num);
            System.out.println("We return  --> "+ 0);
            return response;
        }

        System.out.println();
        System.out.println("Number is NOT less than 2 --> "+ num);
        for (int x = 0; x < binaryString.length(); x++) {
            System.out.println();
            System.out.println("Initial Count --> "+initialCount);
            System.out.println("Greater Count --> "+greaterCount);
            if(binaryString.charAt(x) == '1'){
                System.out.println("Character at index x --> "+binaryString.charAt(x));

                if (initialCount > greaterCount) {
                    greaterCount = initialCount;
                }
                initialCount = 0;
            }else {
                initialCount ++;
